...
### body
block の中身。`entries` と、先頭・末尾の trivia（`leading` / `trailing`）から成る。
- コード: Annotated.swift
- 関連: [[block]] [[entry]] [[trivia]]

### parse
nested・strict な読み（chord 経路）。dotted key を畳み、`[​[a.b]]` を nested に扱い、
...
### Row
`parse` の `Value.arrayOfTables` が持つ AoT 1 行。行の `fields` と `[​[header]]` の
...
### AoT
array-of-tables（`[​[a.b]]`）の略。同名ヘッダの繰り返しが要素列になる。
...
### Toml.Value
lossy・**凍結**された consumer 向け値モデル。5 つの家族アプリが import する射影で、
